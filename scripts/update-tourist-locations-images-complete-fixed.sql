...
-- 1. Fixes GIN index issue with JSON datatype
-- 2. Fixes JSON function usage
-- 3. Uses proper JSON validation constraint
...
-- Data extracted from specs/tourist_attractions.json
...
-- Step 5: Add JSON validation constraint (PostgreSQL compatible)
...
-- Step 6: Create index for better performance (fixed approach)
-- Using expression index for compatibility across PostgreSQL versions
...
-- For PostgreSQL 9.4+ with advanced JSON queries, you can alternatively use:
-- First convert to JSONB for better GIN index support:
-- ALTER TABLE tourist_locations ALTER COLUMN firebase_storage_images TYPE JSONB USING firebase_storage_images::jsonb;
...
-- Verification queries (using correct JSON functions)
SELECT 
    place_name, 
...
-- Check that all expected locations were updated
SELECT 
    CASE 
        WHEN COUNT(*) = 133 THEN ''SUCCESS: All 133 locations updated''
        ELSE ''WARNING: Only '' || COUNT(*) || '' out of 133 locations updated''
