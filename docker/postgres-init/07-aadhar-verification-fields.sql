...
-- NOTE: the original scripts/add-aadhar-verification-fields.sql has a trailing
-- "INSERT INTO verification_logs (message) VALUES (...)" changelog note here —
-- dropped from this local-dev init copy since it references a table that's
-- never created anywhere in scripts/ (the table actually created above is
-- "aadhar_verification_logs", not "verification_logs" — looks like a copy-paste
-- typo in the original script; harmless to drop since it's just an audit note,
