...
-- NOTE: the original scripts/create-bookings-table.sql has a trailing sample
-- INSERT here that assumes homestays/homestay_rooms already have rows (it
-- subselects "LIMIT 1" from each) — dropped from this local-dev init copy
-- since it fails with a null-constraint violation against an empty schema.